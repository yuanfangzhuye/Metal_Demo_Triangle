...
- (instancetype)initWithMetalKitView:(MTKView *)mtkView {
    ...
    if (self) {
        ...
        //2.在项目中记载所有的(.metal)着色器文件
        //从 bundle 中获取 .metal 文件
...
- (void)drawInMTKView:(nonnull MTKView *)view {
    ...
    if (renderPassDescriptor != nil) {
        ...
        [renderEncoder setVertexBytes:triangleVetices length:sizeof(triangleVetices) atIndex:LcVertexInputIndexVertices];
        
        //viewPortSize 数据
        //1) 发送到顶点着色函数中,视图大小
        //2) 视图大小内存空间大小
        //3) 对应的索引
        [renderEncoder setVertexBytes:&_viewportSize length:sizeof(_viewportSize) atIndex:LcVertexInputIndexViewportSize];
